...
class BasicSort(object):
	...
	# 选择排序
	def selectionSort(self, seq=None, reversed=False):
		if isinstance(seq, Iterable):
			# 函数可以接受传来的sequence
			self.seq = seq
		lens = len(self.seq)
	...
	# 插入排序
	def insertionSort(self, seq=None, reversed=False):
		if isinstance(seq, Iterable):
			# 函数可以接受传来的sequence
			self.seq = seq
		lens = len(self.seq)
	...
	# 快速排序
	def quickSort(self, seq, start, end):
		if start < end:
	...
	def partition(self, seq, start, end):
		pivot_index = start - 1
...
if __name__ == "__main__":
	...
	print(basicSort.insertionSort(reversed=True))
